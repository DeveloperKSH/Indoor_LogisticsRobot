...
class WheelControl(smach.State):
    ...
    def execute(self, userdata):
        ...
        while rclpy.ok():
            if self.function == 'MoveDelta':
                ...
                current_bearing = userdata.blackboard.bearing
                debug(f"Current bearing: {current_bearing:.2f} degrees")
                # rotation_angle = self.value - userdata.blackboard.bearing # bearing N 0, E 90, S 180, W 270 based
                rotation_angle = (self.value - current_bearing + 360) % 360
                if rotation_angle > 180:
                    rotation_angle -= 360  # 최소 회전 각도로

                debug(f"Rotation angle to east: {rotation_angle:.2f} degrees")
    ...
    def move(self, function, value):
        debug(f'WheelControl - Function: {function}, Value: {value:.4f} rad')

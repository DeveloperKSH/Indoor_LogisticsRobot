...
class FleetManager(smach.State):
    def __init__(self, node, config, timeout=None):
        ...
        self.charger_name = self.config["rmf_fleet"]["robots"][self.robot_name].get("charger", None)
        self.destinations = self.config["rmf_fleet"]["robots"][self.robot_name].get("destination", [])
        self.previous_status = None  # 이전 상태를 저장할 변수 추가
        mgr_config = self.config['fleet_manager']

        info(f"charger_name: {self.charger_name}")
        info(f"destinations: {self.destinations}")
    ...
    def robot_state_cb(self, msg):
        if (msg.name in self.robots):
            ...
            if (msg.mode.mode == RobotMode.MODE_IDLE or msg.mode.mode == RobotMode.MODE_CHARGING) and len(
                    msg.path) == 0:
                robot = self.robots[msg.name]
                robot.destination = None
    ...
    def execute(self, userdata):
        ...
        ros2 topic pub -1 --keep-alive 5 /rmf_patrol_request std_msgs/msg/String "data: 'iot_task_request,door,0'" --qos-durability transient_local
    ...
    def on_message(self, ws, message):
        ...
        try:
            ...
            if package.get('type') == 'connected':
                ...
                elif task_type == "default_robot_task_request":
                    status = data.get("status")
                    if status != self.previous_status:
                        if status == "underway":
                            info(f'Task status changed to underway')
                            # play sound, music
                            self.publish_sound_file('/root/scripts/sound/sounds/voice/next_waypoint.mp3')
                        elif status in ["canceled", "completed"]:
                            info(f'Task status changed to {status} place {data.get("place")}')
                            # play sound, completed 멘트
                            self.publish_sound_file('/root/scripts/sound/sounds/voice/arrived_destination.mp3')
    ...
    def rmf_patrol_request_callback(self, msg):
        ...
        if task_type == "robot_task_request" or task_type == "iot_task_request":
            request_data = {
                "fleet_name": self.fleet_name,
                "robot_name": self.robot_name,
                "type": task_type,
            ...
            if task_type == "iot_task_request" and category == "door" and self.destinations:
                request_data["value"] = ",".join(self.destinations)

            temp = [{'request': request_data}]
            formatted_data = json.dumps(temp)
            info(f'formatted_data: {formatted_data}')
            if self.connection_open:
                try:

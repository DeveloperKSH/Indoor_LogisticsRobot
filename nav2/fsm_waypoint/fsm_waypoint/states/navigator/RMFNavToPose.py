...
# from tf2_ros import Buffer, TransformListener, TransformException
from nav2_msgs.action import NavigateToPose, FollowGPSWaypoints, FollowWaypoints
...
class RMFNavToPose(smach.State):
    def __init__(self, node, config, timeout=0):
        ...
        self.action_client = ActionClient(self.node, FollowGPSWaypoints, 'follow_waypoints')
        ...
        self.pub1_ = self.node.create_publisher(RobotState,
                                               '/robot_state',
                                               qos_profile=transient_qos)
        self.pub2_ = self.node.create_publisher(DiagnosticArray, '/diagnostics', 10)

        self.pub3_ = self.node.create_publisher(Float64,
                                                '/desired_speed',
                                                qos_profile=transient_qos)
        ...
        self.timer = self.node.create_timer(1.0, self.publish_diagnostics)
        self.state_timer = self.node.create_timer(0.5, self.update_robot_state_without_gps)

    # def send_goal(self, destination_pose, task_id):
    #     if not self.action_client.wait_for_server(timeout_sec=5.0):
    #         error("Action server not available for follow_gps_waypoints")
    #         return

    #     goal_msg = FollowGPSWaypoints.Goal()
    #     goal_msg.gps_poses = destination_pose
    ...
    def send_goal(self, pose_stamped: PoseStamped, task_id):
        if not self.action_client.wait_for_server(timeout_sec=5.0):
    ...
    def execute(self, userdata):
        ...
        self.sub3_ = self.node.create_subscription(Float32MultiArray,
                                              "/tr500_power",
                                              self.battery_callback,
                                              qos_profile=transient_qos
                                              )
        self.sub4 = self.node.create_subscription(
                                                PoseWithCovarianceStamped,
                                                '/amcl_pose',  # 또는 '/robot_pose'
        ...
        self.node.destroy_subscription(self.sub3_)
        # self.node.destroy_timer(self.timer) 계속 유지
        return outcome

    def publish_diagnostics(self):
        if self.callback_intervals:
            avg_interval = sum(self.callback_intervals) / len(self.callback_intervals)
            current_hz = 1.0 / avg_interval if avg_interval > 0 else 0.0
        else:
            current_hz = 0.0  # 콜백이 호출되지 않았을 경우

        self.callback_intervals = []  # 측정값 초기화
        ...
        status.name = "rmf_robot"
        status.hardware_id = "rmf_robot"
        if current_hz == 0:
            status.level = DiagnosticStatus.ERROR  # 2 (ERROR)
            status.message = "No messages received"
        elif current_hz < 10:
            status.level = DiagnosticStatus.WARN  # 1 (WARN)
            status.message = f"High frequency: {current_hz:.2f} Hz"
        else:
            status.level = DiagnosticStatus.OK  # 0 (OK)
            status.message = f"Normal frequency: {current_hz:.2f} Hz"

        kv1 = KeyValue(key="Actual frequency (Hz)", value=f"{current_hz:.2f}")
        kv2 = KeyValue(key="robot_name", value=f"{self.fleet_name}/{self.robot_name}")

        status.values = [kv1, kv2]
    ...
    def gps_callback(self, msg):
        ...
        self.update_robot_state(gps_data)
    ...
    def amcl_pose_callback(self, msg: PoseWithCovarianceStamped):
        ...
        yaw = robot_to_rmf_yaw(orientation[2])
        self.makeup_robot_state_publisher(x, y, yaw)

    def battery_callback(self, msg):
        if len(msg.data) >= 2:
            power_data = {
                "voltage_V": msg.data[0],
                "charge": msg.data[1]
            }
            # info(f"Received power data: {power_data}")
            self.battery_level = power_data["charge"]
    
    def update_robot_state_without_gps(self):
        transform_stamped = self.get_current_transform()
        if not transform_stamped:
            return

        # 1. 위치 및 orientation 추출
        ...
        current_location.yaw = yaw
        current_location.t = self.node.get_clock().now().to_msg()
        current_location.level_name = "L1"  # 실내 맵의 층 이름
        ...
        robot_state_msg.battery_percent = float(self.battery_level)
        ...
        # 5. 퍼블리시
        self.pub1_.publish(robot_state_msg)

        # 6. 진단 주기 계산용 (옵션)
        self.tick_count += 1
        current_time = self.node.get_clock().now().nanoseconds / 1e9  # 초 단위
    ...
    def update_robot_state(self, gps_data):
        transform_stamped = self.get_current_transform()
        if transform_stamped:
            ...
            self.tick_count += 1
            # 현재 시간 가져오기
            current_time = self.node.get_clock().now().nanoseconds / 1e9  # 초 단위 변환
            if self.last_callback_time is not None:
                interval = current_time - self.last_callback_time
                self.callback_intervals.append(interval)
    ...
    #             self.pub3_.publish(desired_speed_msg)
    #         else:
    ...
    #         # target_yaw_robot = rmf_to_robot_yaw(msg.path[1].yaw) not used
    ...
    #         # info(f'current_wgs84_coords: {current_wgs84_coords}, target_wgs84_coords: {target_wgs84_coords}')
    #         # info(f'target_yaw_robot: {target_yaw_robot:.3f}')
    ...
    #         geopose_wps = [latLonYaw2Geopose(target_wgs84_coords['lat'], target_wgs84_coords['lon'], target_yaw_robot)]
    ...
    def path_request_cb(self, msg):
        try:
            # 중복 task 처리 방지
            if self.ongoing_request_cmd_id and self.ongoing_request_cmd_id == msg.task_id:
                warning("Task already in progress. Cancelling previous task.")
                self.cancel_goal(self.ongoing_request_cmd_id)
                return

            if len(msg.path) < 2:
                warning("Invalid path received. At least 2 waypoints are required.")
                return
            ...
            speed_to_publish = None
            for location in msg.path:
                if location.obey_approach_speed_limit:
                    speed_to_publish = location.approach_speed_limit
                    break
            if speed_to_publish is None and len(msg.path) > 0:
                speed_to_publish = msg.path[0].approach_speed_limit
            ...
            info(f"Sending NavigateToPose goal: x={target.x:.2f}, y={target.y:.2f}, yaw={yaw:.2f}")
            self.send_goal(pose, msg.task_id)

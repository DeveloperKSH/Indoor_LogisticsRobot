...
class CheckLifecycleStates(smach.State):
    ...
    def execute(self, userdata):
        ...
        self._inactive_counts = {}  # Track inactive counts for each node
                ...
                try:
                    ...
                    if future.result() is not None:
                        ...
                        if state.label != 'active':
                            ...
                            # Initialize or increment inactive count
                            if node_name not in self._inactive_counts:
                                self._inactive_counts[node_name] = 0
                            self._inactive_counts[node_name] += 1
                            info(
                                f"{node_name} is in state: {state.label} (inactive count: {self._inactive_counts[node_name]})")

                            # Check if we've reached the threshold
                            if self._inactive_counts[node_name] >= 100:
                                warning(f"{node_name} has been inactive for 100 checks. Restarting nav2...")
                                try:
                                    import subprocess
                                    subprocess.run(["pm2", "start", "1"], check=True)
                                    info("Successfully executed: pm2 start 1")
                                    self._inactive_counts[node_name] = 0  # Reset counter after restart
                                except subprocess.CalledProcessError as e:
                                    error(f"Failed to restart nav2: {str(e)}")
                                break

                        else:
                            # Reset counter if node is active
                            if node_name in self._inactive_counts:
                                self._inactive_counts[node_name] = 0

...
from fsm_waypoint.tasks import wss_command
from fsm_waypoint.utils import child_termination_cb, outcome_cb


def ready(node):
    sm = smach.Concurrence(outcomes=['bearing', 'waypoint', 'path', 'create', 'update', 'cancel', 'rmf',
    ...
    with sm:
        ...
        smach.Concurrence.add('WSS_COMMAND', wss_command(node))
